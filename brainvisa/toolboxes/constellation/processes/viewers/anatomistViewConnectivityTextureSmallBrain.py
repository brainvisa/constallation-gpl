...
signature = Signature(
    'connectivity_matrix', ListOf(
                               ReadDiskItem('Group Matrix',
                                            'aims readable volume formats')),
    'mesh', ReadDiskItem('BothAverageBrainWhite', 'Aims mesh formats'),
    'basins_texture', ListOf(ReadDiskItem('Avg Filtered Watershed', 
...
#            graph["lateralized_view"] = self.lateralized_view
        ...
        for rnum in regions:
            ...
            aims.GraphManip.storeAims(graph,
                                      vertex,
                                      "aims_Tmtktri",
                                      aims_mesh)
        ...
        for (i, vertex) in enumerate(graph.vertices()):
            ...
            textured_mesh = a.fusionObjects([mesh_copy, ana_basins_texture],
    ...
    # fusion between mesh and texure
    major_textured_mesh = a.fusionObjects([ana_mesh, ana_major_texture],
                                          method="FusionTexSurfMethod")
    ...
    # control on objects
    a.execute("SetControl",
              windows=[win1],
              control="SmallBrainsControl")
