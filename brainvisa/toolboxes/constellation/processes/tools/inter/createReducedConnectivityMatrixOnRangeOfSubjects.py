...
def initialization(self):
    ...
    def linkWatershed(self, dummy):
        if (self.group is not None and self.patch_label
        and self.texture_group and self.study):
    ...
    self.linkParameters(
        "complete_connectivity_matrix", (
            "group", "study", "patch_label", "smoothing"),
        linkIndividualMatrices)
    self.linkParameters(
        "filtered_watershed", (
            "group", "patch_label", "texture_group", "study", "smoothing"),
        linkWatershed)
    self.linkParameters(
        "reduced_connectivity_matrix", (
            "complete_connectivity_matrix", "texture_group", "group",
            "smoothing"), linkProfiles)
    ...
    eNode = ParallelExecutionNode(
        ...
        notify=True)
    eNode = ParallelExecutionNode(
        "gyri_texture", parameterized=self,
        possibleChildrenProcesses=["createReducedConnectivityMatrix"],
        notify=True)
    ...
    eNode.addLink(
        None, "gyri_texture",
        partial(brainvisa.processes.mapValuesToChildrenParameters, eNode,
                eNode, "gyri_texture", "gyri_texture",
                defaultProcess="createReducedConnectivityMatrix",
                name="createReducedConnectivityMatrix"))