...
def execution(self, context):
    ...
    """
    # TODO: put condition for self.patch_label...
    if self.patch_label is None:
    ...
    context.write(str([os.path.basename(i.fullPath())
                  for i in self.reduced_connectivity_matrix]))

    # show list of gyri segmentation
    context.write(str([i.fullPath() for i in self.gyri_texture]))
    ...
    for y in self.gyri_texture:
        cmd_args += ["-t", y]
    cmd_args += ["-m", self.average_mesh, "-l", str(patch_label),
